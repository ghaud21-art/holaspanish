...
export async function suggestPracticeSentenceWithGemini({ level, vocabHint, recentPrompts = [] }) {
  ...
  const recentText = recentPrompts.length
    ? `아래는 최근에 이미 냈던 연습 문장들입니다. 이것들과 주제·상황·소재가 겹치지 않는 완전히 새로운
문장을 만들어주세요 (예: 최근에 "카페"가 나왔다면 이번엔 카페 이야기를 피하세요).
...
매번 다른 소재(가족, 여행, 취미, 날씨, 일/공부, 음식, 감정, 계획 등)를 다양하게 골고루 활용해서
지루하지 않고 신선하게 느껴지도록 해주세요. 너무 어려운 단어나 학습자가 배우지 않았을 법한 문법은
피해주세요. 스페인어 번역은 절대 포함하지 말고 한국어 문장만 응답하세요.
