...
const PRACTICE_STYLES = [
  { name: '지난 일 이야기하기', desc: '어제/지난 주말에 있었던 일을 순서대로 이야기하는 서술문 (과거 시제 위주)' },
  { name: '부탁·제안하기', desc: '누군가에게 무언가를 부탁하거나 같이 하자고 제안하는 대화체 문장' },
  { name: '생각·느낌 말하기', desc: '어떤 것에 대한 자신의 의견이나 느낌을 말하는 문장' },
  { name: '계획 말하기', desc: '앞으로의 계획이나 약속을 이야기하는 문장 (미래 시제 위주)' },
  { name: '질문과 대답', desc: '한 사람이 묻고 다른 사람이 답하는 짧은 대화 형식' },
  { name: '하루 일과 설명', desc: '오늘 하루 일과를 순서대로 설명하는 문장' },
  ...
  { name: '비교하기', desc: '두 가지를 비교하면서 어느 쪽이 더 좋은지 이야기하는 문장' },
];

// 지금까지 완료한 챕터의 어휘 범위 안에서, 스페인어 여러 문장으로 번역하며 연습하기 좋은
// 조금 더 긴 한국어 문장(3~4문장)을 추천해줘요. 스페인어 번역은 절대 포함하지 않아요.
export async function suggestPracticeSentenceWithGemini({ level, vocabHint, recentPrompts = [] }) {
  const ai = getClient();
  if (!ai) throw new Error('Gemini API가 설정되지 않았습니다.');

  const style = PRACTICE_STYLES[Math.floor(Math.random() * PRACTICE_STYLES.length)];
...
- 너무 어려운 단어나 학습자가 배우지 않았을 법한 문법은 피해주세요.
