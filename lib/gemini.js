...
// 지금까지 완료한 챕터의 어휘 범위 안에서, 스페인어 두 문장으로 번역하며 연습하기 좋은
// 한국어 문장(2문장)을 추천해줘요. 스페인어 번역은 절대 포함하지 않아요.
export async function suggestPracticeSentenceWithGemini({ level, vocabHint }) {
  const ai = getClient();
  if (!ai) throw new Error('Gemini API가 설정되지 않았습니다.');
...
위 단어들과 비슷한 난이도로, 학습자가 스페인어 두 문장으로 번역해서 연습하기 좋은 자연스러운
한국어 문장 두 개(짧게 이어지는 상황)를 만들어주세요. 너무 어려운 단어나 학습자가 배우지 않았을
법한 문법은 피해주세요. 스페인어 번역은 절대 포함하지 말고 한국어 문장만 응답하세요.

아래 JSON 형식으로만 답하세요.
{"kr": "한국어 문장 1. 한국어 문장 2."}`;
