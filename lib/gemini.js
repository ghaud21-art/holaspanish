...
배웠어요. 반드시 현재 시제와 아주 기본적인 문장 구조만 사용하세요.`;
  ...
  const instruction = `당신은 한국인 스페인어 학습자를 위한 독해 문제를 만드는 선생님입니다.
...
위 범위 안에서, 학습자가 읽고 이해할 수 있는 짧은 스페인어 지문을 만들어주세요:
- 4~6개의 문장으로 이루어진 하나의 이야기나 설명 (일상생활, 취미, 가족, 여행, 일과 등 다양한 소재 중 하나)
- 위에서 정해진 문법 범위를 절대 넘지 마세요
- 너무 어려운 단어는 피하세요

그리고 이 지문의 내용을 제대로 읽었는지 확인하는 객관식 질문을 3개 만들어주세요:
- 질문과 보기는 모두 한국어로 작성하세요 (지문 내용에 대한 이해도를 확인하는 질문)
- 각 질문은 보기 4개 중 하나만 정답이어야 합니다
- 지문에 없는 내용을 묻지 마세요
